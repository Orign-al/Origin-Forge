...
版本：`PORTAL-5A-FULL-PLATFORM-RELEASE + RENEWAL/RESTORE POLICY + MULTI-GPU/HIGH-MEMORY APPROVAL`
...
- Job 内存不超过 32 GiB 时直接提交；更高内存必须提供用途、预计内存拆分与必要性说明并等待管理员审批。
- 每个用户有 300 GiB 私有持久化存储，`/workspace` 与 `/home/<用户名>` 都属于同一用户配额。
- Lease 有效期为 96 小时。续期和回收站恢复都遵循管理员为该用户设置的独立审批策略；到期回收不会立即删除持久化数据。
- 管理员可以按用户启用容器内受控免密 sudo；该权限不会全局开启，也不会提供宿主机或 GPU 访问。
...
管理员可以按用户启用受控免密 sudo。已有容器需要仅重建该用户的容器一次并产生短暂中断；
`/workspace`、`/home/<用户名>`、Lease、SSH 访问和 300 GiB 配额保持不变。启用后可用
`sudo -n id -u` 验证，预期输出 `0`。sudo 只获得容器内 root，不提供宿主 root、Docker
Socket、MUNGE、Worker Socket、host namespace 或额外 GPU。
...
Job 内存不超过 32 GiB 时无需额外申请。更高内存必须填写任务用途、预计内存用量拆分和
必要性说明；申请在管理员审批前不会进入 Worker 或 Slurm。管理员可以降低批准内存，不能
批准超过用户申请的值。单节点允许的最大申请为 486377 MiB。若同一 Job 同时申请多 GPU
和高内存，两项都通过后才会创建唯一的 Slurm Job。
...
- Lease 时长：96 小时。
- 续期窗口：到期前 24 小时在“我的环境”中开放“申请续期”。
- 续期策略：页面显示该用户是需要管理员审批，还是在后端完整校验后自动批准。
- 回收站恢复：与同一用户的续期审批策略挂钩；需要审批时，提交恢复申请不会自动恢复容器。
- 自动批准仍执行续期/恢复时限、Lease 锁、重复请求和资源策略校验。
...
若 Lease 已到期，尚未处理的旧续期申请会变成已失效历史，不能再批准。用户应从“回收站”
提交正式恢复申请，并按该用户当前审批策略等待审批或自动处理。
...
当前容器尚未启用平台受控 sudo。不要设置、索取或共享容器密码；请联系管理员为你的账号
单独启用。已有容器会短暂中断并仅重建一次。完成后运行 `sudo -n id -u`，预期输出 `0`。
