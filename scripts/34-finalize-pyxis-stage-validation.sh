...
if [[ ${EUID} -eq 0 ]]; then
    printf '%s\n' 'Run this script as codexops; it finalizes existing staged audit evidence.' >&2
    exit 2
fi

run_id=${1:?usage: 34-finalize-pyxis-stage-validation.sh RUN_ID}
...
if [[ ! -d ${record_dir} || ! -f ${stage_plugin} \
    || ! -s ${record_dir}/srun-help-stage.txt \
    || ! -f ${record_dir}/srun-help-stage.stderr \
    || ! -s ${record_dir}/srun-container-help-stage.stderr ]]; then
    printf '%s\n' 'The reviewed staged Pyxis evidence is incomplete.' >&2
    exit 1
fi
if [[ -e ${report_file} ]]; then
    printf '%s\n' 'Refusing to overwrite the Pyxis staged build report.' >&2
    exit 1
fi
...
if [[ $(squeue -h | wc -l) != 0 ]]; then
    printf '%s\n' 'Jobs exist unexpectedly during staged Pyxis finalization.' >&2
    exit 1
fi
if [[ -s ${record_dir}/srun-help-stage.stderr ]]; then
    printf '%s\n' 'The successful staged srun help probe wrote stderr.' >&2
    exit 1
fi
if ! grep -Fq -- '--container-image' "${record_dir}/srun-help-stage.txt" \
    || ! grep -Fq '[pyxis]' "${record_dir}/srun-help-stage.txt"; then
    printf '%s\n' 'The staged srun help lacks Pyxis option registration.' >&2
    exit 1
fi
if ! grep -Fxq "srun: unrecognized option '--container-help'" \
    "${record_dir}/srun-container-help-stage.stderr"; then
    printf '%s\n' 'The optional container help failure differs from the reviewed result.' >&2
    exit 1
fi
if grep -FRqi 'Incompatible plugin version' \
    "${record_dir}/srun-help-stage.txt" "${record_dir}/srun-help-stage.stderr" \
    "${record_dir}/srun-container-help-stage.txt" \
    "${record_dir}/srun-container-help-stage.stderr"; then
    printf '%s\n' 'Incompatible plugin version detected in staged evidence.' >&2
    exit 1
fi
