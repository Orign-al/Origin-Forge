...
run_id=${1:?usage: 28-finalize-enroot-install.sh RUN_ID}
...
record_dir=${platform_dir}/reports/enroot-install-finalize-${run_id}.d
...
if [[ ! -d ${platform_dir}/reports/enroot-install-${run_id}.d ]]; then
    printf '%s\n' 'The stopped Enroot installation record is missing.' >&2
    exit 1
fi
if [[ -e ${record_dir} || -e ${report_file} ]]; then
    printf '%s\n' 'Refusing to overwrite Enroot finalization records.' >&2
    exit 1
fi
...
if ! sudo cmp -s "${config_source}" "${config_target}"; then
    printf '%s\n' 'Installed Enroot platform drop-in differs from the reviewed source.' >&2
    exit 1
fi
if [[ $(cat /proc/sys/kernel/apparmor_restrict_unprivileged_userns) != 1 ]]; then
    printf '%s\n' 'The Ubuntu unprivileged-userns AppArmor restriction was weakened.' >&2
    exit 1
fi
sudo aa-status >"${record_dir}/apparmor-status.txt"
if ! grep -Fq '/usr/bin/enroot-nsenter' "${record_dir}/apparmor-status.txt"; then
    printf '%s\n' 'The official Enroot AppArmor profile is not loaded.' >&2
    exit 1
fi
...
for parent in cache data runtime; do
    ...
    if [[ $(sudo stat -c '%U:%G:%a' "${parent_dir}") != root:root:711 ]]; then
        printf 'Enroot parent directory metadata is invalid: %s\n' "${parent_dir}" >&2
        exit 1
    fi
    if [[ $(sudo stat -c '%U:%G:%a' "${user_dir}") != codexops:codexops:700 ]]; then
        printf 'Enroot per-UID directory metadata is invalid: %s\n' "${user_dir}" >&2
        exit 1
    fi
...
if [[ ${node_state} != drain* && ${node_state} != drained* ]]; then
    printf '%s\n' 'Slurm node left DRAIN during Enroot finalization.' >&2
    exit 1
fi
if [[ $(squeue -h | wc -l) != 0 ]]; then
    printf '%s\n' 'Jobs exist unexpectedly after Enroot finalization.' >&2
    exit 1
fi
