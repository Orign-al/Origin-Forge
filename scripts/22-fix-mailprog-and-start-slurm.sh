...
run_id=${1:?usage: 22-fix-mailprog-and-start-slurm.sh RUN_ID}
if [[ ! ${run_id} =~ ^[0-9]{8}-[0-9]{6}$ ]]; then
    printf 'Invalid RUN_ID: %s\n' "${run_id}" >&2
    exit 2
fi

readonly old_config_sha=4bcb3a4839e34c1983aad465cfe3b472b2143f31ec5e3fb1482fef0ce7c9ba40
readonly new_config_sha=51c0ccf4e601d850d4b737d6a8f1408dea576160b08daaa9491c7aa20131028d
script_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)
platform_dir=$(dirname -- "${script_dir}")
record_dir=${platform_dir}/reports/slurm-mailprog-fix-${run_id}.d
report_file=${platform_dir}/reports/slurm-base-${run_id}.md
source_config=${platform_dir}/config/slurm.conf
target_config=/etc/slurm/slurm.conf
backup_config=${platform_dir}/backups/slurm.conf-before-mailprog-${run_id}
hostname_expected=sagsh100server
configured_memory=486377
reserved_memory=24576

if [[ ! -d ${platform_dir}/reports/slurm-base-start-${run_id}.d ]]; then
    printf '%s\n' 'The MailProg validation failure record is missing.' >&2
    exit 1
fi
if [[ -e ${record_dir} || -e ${report_file} || -e ${backup_config} ]]; then
    printf '%s\n' 'Refusing to overwrite a MailProg fix record, report, or backup.' >&2
    exit 1
fi
...
if [[ ! -x /usr/bin/true ]]; then
    printf '%s\n' 'The selected no-op MailProg is not executable.' >&2
    exit 1
fi
if ! grep -Fxq 'MailProg=/usr/bin/true' "${source_config}"; then
    printf '%s\n' 'The reviewed source configuration lacks the explicit no-op MailProg.' >&2
    exit 1
fi
if [[ $(sha256sum "${source_config}" | awk '{print $1}') != "${new_config_sha}" ]]; then
    printf '%s\n' 'The reviewed source configuration hash changed.' >&2
    exit 1
fi
if [[ $(sudo sha256sum "${target_config}" | awk '{print $1}') != "${old_config_sha}" ]]; then
    printf '%s\n' 'The installed pre-fix Slurm configuration hash changed.' >&2
    exit 1
fi
for unit in slurmctld slurmd; do
    if systemctl is-active --quiet "${unit}"; then
        printf 'Service unexpectedly active before the MailProg fix: %s\n' "${unit}" >&2
        exit 1
    fi
done
...
sudo chown root:gpu-platform-admin "${backup_config}"
sudo chmod 0640 "${backup_config}"
sudo install -o root -g root -m 0644 "${source_config}" "${target_config}"
if [[ $(sudo sha256sum "${target_config}" | awk '{print $1}') != "${new_config_sha}" ]]; then
    printf '%s\n' 'Installed Slurm configuration hash does not match the reviewed fix.' >&2
    exit 1
fi

set +e
diff -u "${backup_config}" "${source_config}" >"${record_dir}/slurm-conf-change.diff"
diff_status=$?
set -e
if [[ ${diff_status} != 1 ]] || [[ $(grep -c '^+MailProg=/usr/bin/true$' "${record_dir}/slurm-conf-change.diff") != 1 ]]; then
    printf '%s\n' 'The installed change is not limited to the reviewed MailProg addition.' >&2
    exit 1
fi

sudo slurmd -G >"${record_dir}/slurmd-G.txt" 2>&1
if grep -Eqi 'error:|fatal:|invalid|mismatch|ignored' "${record_dir}/slurmd-G.txt"; then
    printf '%s\n' 'slurmd -G reported a GRES validation problem after the config fix.' >&2
    exit 1
fi
...
sudo -u slurm timeout --signal=TERM --kill-after=5s 12s \
    /usr/sbin/slurmctld -D -c -f "${target_config}" -vv \
...
if grep -Eqi 'fatal:|error:' "${record_dir}/slurmctld-foreground-validation.txt"; then
    printf '%s\n' 'slurmctld foreground validation still reported a fatal or error condition.' >&2
    exit 1
fi
...
service_start=$(date --iso-8601=seconds)
...
sudo journalctl -u slurmctld -u slurmd --since "${service_start}" --no-pager \
