...
Portal 是单节点、受控虚拟网络内的管理控制面。Web 在 IPv4 用户入口监听，由 systemd
网络策略把来源限制到批准的 VPN、EasyTier 和管理 LAN 网段；用户正式地址为
`20.10.10.3:18080`。API 仍只监听 `127.0.0.1:18081`，PostgreSQL 仍只使用本机 Unix
Socket。SSH Tunnel 仅为可选回退。管理员已接受当前 Pilot 使用内部明文 HTTP；TLS 未
启用且当前范围不要求启用。Portal 不提供公网入口，也不把当前入口描述为 HTTPS。

```text
浏览器（EasyTier 20.10.10.3；内部 HTTP；SSH Tunnel 可选）
        |
        v
Next.js Web IPv4 :18080（用户地址 20.10.10.3）
                  -- /api/* --> FastAPI 127.0.0.1:18081
                                      |
                   +------------------+------------------+
                   |                                     |
                   v                                     v
