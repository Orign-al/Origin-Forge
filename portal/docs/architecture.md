...
Portal-0/1 是单节点、私有隧道网络内的管理控制面。经管理员批准，Web 精确监听服务器
`tun0` 地址 `10.10.10.2:18080`；API 仍只监听 `127.0.0.1:18081`，PostgreSQL 仍只
使用本机 Unix Socket。浏览器可从批准的 `10.10.10.0/24` 私有隧道直接访问 Web，SSH
Tunnel 仍是回退方式。Portal 不修改防火墙、不监听 `0.0.0.0`、不提供公网入口，也不把
当前 HTTP 模式描述为公开生产 HTTPS。

```text
浏览器（批准的 10.10.10.0/24 私有隧道；SSH Tunnel 可选）
        |
        v
