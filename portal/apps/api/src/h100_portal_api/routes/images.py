...
def images(
    ...
    live = adapter("images.list", context)
    live_images = live.get("images", []) if live.get("status") in {"OK", "PARTIAL"} else []
    managed_images = [
        {
            "id": str(row.id),
            ...
            "jobs_in_use": None,
            "source": "portal_inventory",
        }
        for row in rows
    ]
    return {
        "status": "OK" if live.get("status") in {"OK", "PARTIAL"} else "PARTIAL",
        "count": len(live_images) + len(managed_images),
        "images": [*live_images, *managed_images],
